...
<html>
        		...
        		<div class="panel panel-default ks-left">
	            	<div class="panel-body">
	            		<ul class="nav nav-pills nav-stacked left_ks" role="tablist">
						 <c:forEach items="${listIlltp }" var="listIlltp">
							  <li><a href="#${listIlltp.illtpId }">${listIlltp.illtpName }<span class="glyphicon glyphicon-chevron-right pull-right"></span></a></li>
						 </c:forEach>
						</ul>
	            	</div>
	            </div>
        				...
        				<c:forEach items="${listIlltp }" var="listIlltp">
	        				...
	        				<div class="panel panel-default">
	        					<div class="panel-heading">
	        						<h6 id="${listIlltp.illtpId }">${listIlltp.illtpName }</h6>
	        					</div>
	        					<div class="panel-body">
	        						<div>
	        							<c:forEach items="${listIlltp.illList }" var="illList">
						        			<div class="piaoyi"><a href="#" >${illList.illName }</a></div>
						        		</c:forEach>
						        	</div>
	        					</div>
	        				</div>
	...
	<script type="text/javascript">
		...
		$(".piaoyi a").bind("click",function(){
			var illName = $(this).html();
			alert(illName)
			//想办法传中文
			location.href ="<%=request.getContextPath() %>/appointment/queryPersonByIllName?illName="+illName;
		});
		//搜索框条件查询
