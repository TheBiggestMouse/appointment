								...
								<select class="form-control" id="left" name="right" size="9" multiple>
									<c:forEach items="${roleList}" var="role">
										<c:if test="${role.selected==false}">
											<option value="${role.label}" ondblclick="moveSide(this)">${role.value}</option>
										</c:if>
									</c:forEach>
								</select>
							</div>
...
<script type="text/javascript">
	...
	function tijiao(){
		...
		location.href = "<%=request.getContextPath()%>/sys/updateAuthUser?roleIds="+rv+"&userId=${user.userId}";
	}
